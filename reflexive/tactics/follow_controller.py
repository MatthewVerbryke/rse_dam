...
class FollowController(object):
    ...
    def input_new_trajectory(self, trajectory):
        ...
        self.desired = list(self.cur_goal.positions)
    ...
    def update(self, state):
        ...
        if time_now + rospy.Duration(0.01) < self.start:
            status = 0
            command = None
            ...
            if time_now + rospy.Duration(0.01) <= self.end:
                command, status = self.calculate_command(state, time_now)
        
            # Handle end time of current goal
            elif time_now + rospy.Duration(0.01) >= self.end:
                self.i += 1
    ...
    def set_next_waypoint(self):
        ...
        self.desired = list(self.trajectory.points[self.i].positions)
        next_t = self.trajectory.points[self.i].time_from_start
        self.end = self.start + next_t
        
    def calculate_command(self, state, t_now):
        """
        ...
        del joint_pos[-1] #<- TODO: improve this
        command = joint_pos
        print "--------------------------------------------------------"
        print "time to next waypoint"
        print (self.end - t_now).to_sec()
        print ""
        ...
        print q_err
        print ""
        dq = [abs(x*self.dt/(self.end - t_now).to_sec()) for x in q_err]
        print "delta"
        print dq
        print ""
